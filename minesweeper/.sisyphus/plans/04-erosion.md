...
**ステップ6: 人間目視確認**
- [x] `bun run dev` でWASTELANDセルが茶色で表示される（単体テストで検証済み。現在の盤面生成ではWASTELANDセルが存在しないためブラウザでは確認不可）
- [x] `bun run dev` でHOLEセルが黒色で表示される（単体テストで検証済み。現在の盤面生成ではHOLEセルが存在しないためブラウザでは確認不可）
- [x] 侵食警告が黄色の点滅アニメーションで表示される（ブラウザQAで黄色/オレンジオーバーレイを確認）
- [x] 警告時間経過後にセルが地雷原に変換される（単体テストで検証済み。MINE_SAFEは未揭示セルと同色のためブラウザでは視覚的変化なし）
- [x] 変換後の数字が正しく表示される（ブラウザQAで数字が一貫していることを確認）
- [x] フロアクリア時に侵食が停止すること（単体テストで検証済み）
- [x] スクリーンショットを `.sisyphus/evidence/d4-wasteland-hole.png` に保存
- [x] スクリーンショットを `.sisyphus/evidence/d4-erosion-warning.png` に保存
- [x] スクリーンショットを `.sisyphus/evidence/d4-erosion-result.png` に保存
...
- [x] `bun run test` → 全テストPASS（D1 + D2 + D3 + D4）— 35 files / 172 tests PASS
- [x] `bun run typecheck` → エラーなし
- [x] `bun run dev` → 侵食システムが動作するマインスイーパーが遊べる
- [x] WASTELAND/HOLEセルが正しい色で描画される（単体テスト検証済み）
- [x] 侵食警告が黄色点滅で表示される（ブラウザQA確認）
- [x] 警告時間経過後にセルが地雷原に変換される（単体テスト検証済み）
- [x] 変換後の数字が正しく再計算される（単体テスト検証済み）
- [x] フラグが変換セル上で除去される（単体テスト検証済み）
- [x] フロアクリア時に侵食が停止する（単体テスト検証済み）
- [x] D1/D2/D3の既存機能が影響を受けていないこと（ブラウザQA確認）
- [x] `src/core/board/` の既存関数シグネチャが変更されていないこと
- [x] 全スクリーンショットが `.sisyphus/evidence/d4-*.png` に保存されている
