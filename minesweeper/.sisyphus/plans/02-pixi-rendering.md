...
**ステップ6: 人間目視確認**
- [x] 左クリックでセルが開く（flood-fill含む）
- [x] 右クリックで旗が立つ/外れる
- [x] 地雷を踏むとゲームオーバー表示
- [x] 全安全セルを開くと勝利表示
- [x] D1と同じゲーム体験がPixiJSで再現されている
- [x] スクリーンショットを `.sisyphus/evidence/d2-grid-interaction.png` に保存
...
**ステップ6: 人間目視確認**
- [x] マウスホイールでズームイン/アウトできる
- [x] マウスドラッグで画面をパンできる
- [x] ズームがMIN/MAX範囲に収まる
- [x] 大きな盤面でも全体像と拡大表示が切り替えられる
- [x] スクリーンショットを `.sisyphus/evidence/d2-camera.png` に保存
...
**ステップ6: 人間目視確認**
- [x] スマホ画面サイズでジョイスティックが表示される（DevToolsで幅縮小して確認）
- [x] ジョイスティックを操作すると方向ベクトルが画面オーバーレイに表示される（console.logは使用禁止）
- [x] Dig/Flagボタンが表示される
- [x] Digボタンをタップするとセルが開く（T2-3のインタラクションに接続）
- [x] Flagボタンをタップすると旗が立つ（T2-3のインタラクションに接続）
- [x] PC画面ではジョイスティック・ボタンが非表示（または表示切替可能）
- [x] スクリーンショットを `.sisyphus/evidence/d2-mobile-controls.png` に保存
...
**ステップ6: 人間目視確認**
- [x] PCでJキーを押すとカーソル位置のセルが開く
- [x] PCでKキーを押すとカーソル位置のセルに旗が立つ
- [x] キーを押しっぱなしにしても連続発火しない
- [x] スクリーンショットを `.sisyphus/evidence/d2-keyboard-input.png` に保存
...
- [x] `bun run test` → 全テストPASS（D1 + D2）
- [x] `bun run typecheck` → エラーなし
- [x] `bun run dev` → PixiJSでマインスイーパーが表示される
- [x] セルタイプが視覚的に区別できる（未開拓=灰色、開拓済み=薄い灰色+数字）
- [x] 左クリックでセルが開く（flood-fill含む）
- [x] 右クリックで旗が立つ/外れる
- [x] マウスホイールでズーム、ドラッグでパンができる
- [x] DevToolsでモバイル表示にした場合、ジョイスティックとボタンが表示される
- [x] PCでJ/Kキーで操作できる
- [x] D1と同じゲーム体験がPixiJSで完全に再現されている
- [x] `src/core/` 配下のファイルが一切変更されていないこと
- [x] 全スクリーンショットが `.sisyphus/evidence/d2-*.png` に保存されている
